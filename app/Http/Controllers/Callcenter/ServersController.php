...
class ServersController extends Controller
{
    ...
    public function _index_(){
		...
		$data['datalist'] = Servers::get_records();
        return view('Callcenter.servers.servers',$data);
    }    
    public function get_data(Request $request){
        if($request->ajax()) {
            ...
            $datalist=Servers::get_record_by_id($item_id);
            $responsestring='';
	...
	public function _edit_(Request $request,$id){
		...
		$datalist=Servers::get_record_by_id($id);
		$data['item_id'] = $id;
		if($datalist==null){
			return abort(500);
		}
		...
		$data['status'] = '_edit';
        return view('Callcenter.servers.serversedit',$data);
    }
	public function _new_(Request $request){
        $data['error_warning'] = '';
        $data['success'] = '';            	
		$data['item_id'] = Servers::get_new_id();	
		$data['srvlist'] = Servers::get_records();		
		$data['status'] = '_new';
		$data['datalist'] = Servers::get_records();
        return view('Callcenter.servers.serversedit',$data);
    ...
    public function _action_(Request $request){
		...
		if(strlen($hd_status)>0){
			if($hd_status=="_edit"){
				...
				$insertVal = Servers::update_record($hd_val);
				if($insertVal=="0"){
				...
				$insertVal = Servers::add_new_record();
				if($insertVal=="0"){
					$data['success']="اطلاعات با موفقیت ذخیره گردید.";
				}
				else if($insertVal=="1"){
			...
			if(isset($req)){
				...
				foreach ($req as $item_id) {
					$res_val=Servers::delete_record($item_id);
					if (strlen($res_val) > 0){	
						$err_suu .= $res_val . "<br />";
					}
				}
