...
class khayerController extends Controller
{
    ...
    public function _index_(Request $request){
        $data['error_warning'] = '';
        $data['success'] = '';            
		$data['datalist'] = khayer::get_records();		
        return view('manage.khayerin.khayer',$data);
    }
	...
	public function _edit_(Request $request,$id){
		...
		$data['usergrplist']=khayer::get_groups_by_id($id);
		$datalist=khayer::get_record_by_id($id);
		if($datalist==null){
			return abort(500);
		}
		else{
		...
		$data['status'] = '_edit';
        return view('manage.khayer.khayeredit',$data);
    }
	public function _new_(Request $request){
		...
		$data['datalist'] = khayer::get_records();			
		$data['status'] = '_new';
        return view('manage.khayerin.khayeredit',$data);
    }
	...
	public function _export(){
		...
		if(count($lstFields)==0)
		{
			...
			$fields=rtrim($fields,",");			
			if($extype=='pdf'){
				$datas['itemlist'] = khayer::get_fields_records($fields,$ddlSort,$sorttype);
		...
		if($filetype=='application/vnd.ms-excel' || $filetype=='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'){
			...
			if(!empty($data) && $data->count()){
				foreach ($data as $key => $value) {	
					$inserres=khayer::insert_imported_data($value);									
					if($inserres!="0"){
    ...
    public function _action_(Request $request){
		...
		if(Input::get('hdExport')=="_export"){
			...
			if(strlen($hd_val)>0){
				$insertVal = khayer::update_record($hd_val);
				if($insertVal=="0"){
				...
				if(isset($req)){
					...
					foreach ($req as $item_id) {
						$res_val=khayer::delete_record($item_id);
						if (strlen($res_val) > 0){	
							$err_suu .= $res_val . "<br />";
						}
					}
					...
					$insertVal = khayer::add_new_record();
					if($insertVal=="0"){
						$data['success']="اطلاعات با موفقیت ذخیره گردید.";
					}
					else if($insertVal=="1"){
