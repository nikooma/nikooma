...
@section('maincontent')
        ...
        <div class="box-header">  
            ...
            <div class="btn-group pull-left">
                ...
                <div class="btn-group">
                <a id="btnAddNew" href="/musiconhold/addnew" type="button" class="btn btn-primary pull-left" data-toggle="tooltip" title="{{ __('callcenter.newitem') }}" data-original-title="{{ __('callcenter.newitem') }}">{{ __('callcenter.newitem') }} <i class="fa fa-plus"></i></a>
                </div>
				@endif	
        ...
        <!-- /.box-header -->
        <form id="frmList" method="POST" action="musiconhold" data-toggle="validator" role="form">                                                           
            {{ csrf_field() }}
        <div class="box-body">
            ...
            <thead>
				<tr>
					...
					<th style='font-size: 13px; font-weight: bold; text-align: right'>نام صدا انتظار</th>
					<th style='font-size: 13px; font-weight: bold; text-align: right'>نرخ صدا</th>
					<th style='font-size: 13px; font-weight: bold; text-align: right'>پخش تصادفی</th>                
					<th style='font-size: 13px; font-weight: bold; text-align: right'>تعداد کانال</th> 
					<th style='font-size: 13px; font-weight: bold; text-align: right'>زمان پخش هرصدا</th> 					
            ...
            <tbody>                
                <?php foreach ($datalist as $item) { ?>
                    <tr>
                        ...
                        <td class="text-right"><?php echo $item->m_rate; ?> </td> 
                        <td class="text-right"><?php echo ($item->m_shuffle==""?"خیر":"بله"); ?> </td> 
						<td class="text-right"><?php echo $item->m_channels; ?> </td> 
						<td class="text-right"><?php echo $item->m_interval; ?> </td> 
						<td class="text-right">																			
								<a id="btnAddSounds" href="/musiconhold/addsounds/<?php echo $item->m_id; ?>" type="button" class="btn btn-primary pull-left" data-toggle="tooltip" title="صداها" data-original-title="صداها"><i class="fa fa-music"></i></a>						
						</td>				
                    </tr>                    
                <?php } ?>
            </tbody>             
...
@section('footerscript')
    ...
    <script>
        function actionDeleteForm(){
            $("#frmList").attr('action', 'musiconhold');
            $("#frmList").submit();
        }     
		@if(Permissions::permissionCheck('23','03')==1)
        $(document).ready(function(){            
            $("#lnkEdit").click(function(){
                ...
                if(item_id!=''){
                    window.location = "/musiconhold/edit/"+item_id;    
                }
            });       
        });
		@endif
