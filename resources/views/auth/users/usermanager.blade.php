...
@section('maincontent')
        ...
        <div class="box-header">  
            ...
            <div class="btn-group pull-left">
                ...
                <div class="btn-group">
                <a id="btnAddNew" href="/manageusers/addnew" type="button" class="btn btn-primary pull-left" data-toggle="tooltip" title="{{ __('callcenter.newitem') }}" data-original-title="{{ __('callcenter.newitem') }}">{{ __('callcenter.newitem') }} <i class="fa fa-plus"></i></a>
                </div>
				@endif
				...
				@endif
				<a class="btn btn-info" onclick="openExport()" data-toggle="tooltip" title="خروجی گزارش" data-original-title="خروجی گزارش" tooltip="خروجی گزارش"><i class="fa fa-external-link-square"></i></a>
				<a class="btn btn-info" onclick="openImport()" data-toggle="tooltip" title="ورودی گزارش" data-original-title="ورودی گزارش" tooltip="ورودی گزارش"><i class="fa fa-upload"></i></a>                                
            </div>          
        </div>
        <!-- /.box-header -->
        ...
        <div class="box-body">
            ...
            <tr>
                ...
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.image') }}</th>
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.itemid') }}</th>
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.namefamily') }}</th>
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.username') }}</th>					
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.mobile') }}</th>
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.email') }}</th>                                            
                <th style='font-weight: bold; text-align: right'>{{ __('callcenter.lastloginip') }}</th> 
				<th style='font-weight: bold; text-align: right'>{{ __('callcenter.lastlogin') }}</th>				
            </tr>
            </thead>
            <tbody>                
                <?php foreach ($datalist as $item) { ?>
                    <tr>
                        ...
                        <td class="text-right">						
                            <img class="esmall" style="width:30px;height:30px;" src="<?php echo $item->u_image; ?>" />                                    						
                         </td> 
						<td class="text-right"><?php echo $item->id; ?> </td>     
                        <td class="text-right"><?php echo $item->name.' '.$item->u_lname; ?> </td> 
                        <td class="text-right"><?php echo $item->username; ?> </td> 												
                        <td class="text-right"><?php echo $item->u_mobilenumber; ?> </td> 
                        <td class="text-right"><?php echo $item->email; ?> </td> 
                        <td class="text-right"><?php echo $item->lastlogip; ?> </td>   
						<td class="text-right"><?php echo $item->lastlogin; ?> </td> 						
                    </tr>                    
                <?php } ?>
            </tbody>             
            </table>
		...
		<form id="frmExport" name="frmexport" method="POST" action="/manageusers" data-toggle="validator" role="form">
			{{ csrf_field() }}
		...
		<form method="POST" name="frmimport" action="/manageusers" enctype="multipart/form-data">
			{{ csrf_field() }}
...
@section('footerscript')
    ...
    <script>
        function actionDeleteForm(){
            $("#frmList").attr('action', 'manageusers');
            $("#frmList").submit();
        }       
		function openExport() {
        ...
        $(document).ready(function(){    			
            $("#lnkEdit").click(function(){
                ...
                if(item_id!=''){
                    window.location = "/manageusers/edit/"+item_id;    
                }
            });       
        });
		@endif
