...
@section('maincontent')
        ...
        <div class="box-header">  
            ...
            <div class="btn-group pull-left">
                ...
                <a href="/niazmandan" class="btn btn-primary" style="margin:10px;">بازگشت <i class="fa fa-reply"></i></a>              
                
				@if(Permissions::permissionCheck('23','05')==1)
                <div class="btn-group">
                <a id="btnAddNew" href="/niazmandan/takafol/<?php echo $takafol_id; ?>/adddew" type="button" class="btn btn-primary" data-toggle="tooltip" title="{{ __('callcenter.newitem') }}" data-original-title="{{ __('callcenter.newitem') }}">{{ __('callcenter.newitem') }} <i class="fa fa-plus"></i></a>
                </div>
				@endif
        ...
        <!-- /.box-header -->
        <form id="frmList" method="POST" action="/niazmandan/takafol/<?php echo $takafol_id; ?>" data-toggle="validator" role="form">                                                           
            {{ csrf_field() }}
        <div class="box-body">
            ...
            <tbody>                
                <?php foreach ($datalist as $item) { ?>
                    <tr>
                        ...
                        <input type="checkbox" name="selected[]" value="<?php echo $item->nt_id; ?>" />
                        </td>                                           
						<td class="text-right"><?php echo $item->nt_meli; ?> </td>     
                        <td class="text-right"><?php echo $item->nt_name.' '.$item->nt_family; ?> </td> 
                        <td class="text-right"><?php echo $item->nt_pedar; ?> </td> 												
                        <td class="text-right"><?php echo $item->nt_yaraneh; ?> </td>   
						<td class="text-right"><?php echo $item->nt_daramad; ?> </td> 
						<td class="text-right"><?php echo $item->nt_tahsil; ?> </td>
						<td class="text-right"><?php echo $item->nt_shoghl; ?> </td>
						<td class="text-right">
							<button id="lnkView" type="button" onclick="openNiazmandData(<?php echo $item->nt_id; ?>)" class="btn btn-primary pull-right" data-toggle="tooltip" title="نمایش" data-original-title="نمایش"><i class="fa fa-search"></i></button>    
						</td>
                    </tr>                    
                <?php } ?>
            </tbody>             
...
@section('footerscript')
    ...
    <script>
        function actionDeleteForm(){
            $("#frmList").attr('action', '/niazmandan/takafol/<?php echo $takafol_id; ?>');
            $("#frmList").submit();
        }       
		function openExport() {
		...
		function openNiazmandData(niaz_id) {
			$.ajax({
				type: "post",
				url: "/niazmandan/takafol/getdata",
				data: {'item_id': niaz_id,'_token': $('input[name=_token]').val()},
				dataType: "json",
				success: function (response) {
					var datas = response;
					var spantext="نام:  "+datas.nt_name+"<br />نام خانوادگی:  "+datas.nt_family+"<br />نام پدر:  "+datas.nt_pedar+
						"<br />کدملی:  "+datas.nt_meli+"<br />تاریخ تولد"+datas.nt_tavalod+"<br />شغل:  "+datas.nt_shoghl+"<br />درآمد: "+datas.nt_daramad+
						"<br />بیمه:  "+datas.nt_bimeh+"<br />میزان تحصیلات:  "+datas.nt_tahsil+"<br />مهارت:  "+datas.nt_maharat+"<br />یارانه:  "+datas.nt_yaraneh+"<br />وضعیت تعهل:  "+(datas.nt_taahol=="1"?"متاهل":"مجرد")+
						"<br />وضعیت جسمانی:  "+datas.nt_jesmani+"<br />جنسیت:  "+(datas.nt_jensiat=="1"?"آقا":"خانم");
        ...
        $(document).ready(function(){    			
            $("#lnkEdit").click(function(){
                ...
                if(item_id!=''){
                    window.location = "/niazmandan/takafol/<?php echo $takafol_id; ?>/edit/"+item_id;
                }
            });       		           
